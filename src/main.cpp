...
#if defined(NDEBUG)
# error "LABH cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "LABH Signed Message:\n";
...
CAmount GetProofOfWorkReward(CAmount nFees)
{
    ...
    } else if (pindexBest->nHeight+1 > 1 && pindexBest->nHeight+1 <= 200) {
        nSubsidy = 1 * COIN;
    } else if (pindexBest->nHeight+1 > 201 && pindexBest->nHeight+1 <= 1000) {
        nSubsidy = 5 * COIN;
...
CAmount GetProofOfStakeReward(const CBlockIndex* pindexPrev, CAmount nCoinAge, CAmount nFees)
{
    ...
    if (pindexPrev->nHeight+1 >= HARD_FORK_VERSION_101)
        nSubsidy *= 100;

    // Force coin cap - return only fees
    if (pindexBest->nMoneySupply + nSubsidy >= MAX_MONEY ||
        pindexBest->nMoneySupply >= MAX_MONEY)
    {
        LogPrint("creation", "GetProofOfWorkReward() : supply=%s create=%s nSubsidy=%d\n", FormatMoney(pindexBest->nMoneySupply), FormatMoney(nFees), nFees);
        return nFees;
    }

    LogPrint("creation", "GetProofOfStakeReward(): coinYearReward=%d blockHeight=%d create=%s nCoinAge=%d\n", GetCoinYearReward(pindexPrev->nHeight+1), pindexPrev->nHeight+1, FormatMoney(nSubsidy), nCoinAge);
...
CBigNum GetWeightSpent(CBlockIndex* pindex)
{
    ...
    CAmount nAmount = txPrev.vout[txInStake.prevout.n].nValue;
...
bool CTransaction::ConnectInputs(CTxDB& txdb, MapPrevTx inputs, map<uint256, CTxIndex>& mapTestPool, const CDiskTxPos& posThisTx,
    const CBlockIndex* pindexBlock, CAmount &nBurnCoins, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    if (!IsCoinBase())
    {
        ...
        if (!IsCoinStake())
        {
            ...
            CAmount nRequiredFee = GetMinFee(*this);
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // LABH: Check address's staking, can't stake more than MinStakingBeforeSuperblock
        if (IlIII(pindex->pprev->nHeight+1) && !vtx[1].lIIII(pindex->pprev->nHeight+1))
            return error("ConnectBlock() : %s unable to stake on superblock for coinstake", vtx[1].GetHash().ToString());
        ...
        CAmount nCalculatedStakeReward = GetProofOfStakeReward(pindex->pprev, nCoinAge, nFees);
...
bool CTransaction::GetCoinAge(CTxDB& txdb, const CBlockIndex* pindexPrev, CAmount& nCoinAge) const
{
    ...
    for (const CTxIn& txin : vin)
    {
        ...
        int64_t nStakeTime = nTime - txPrev.nTime;
        bnCentSecond += CBigNum((nValueIn / COIN) * nStakeTime);

        LogPrint("coinage", "coin age nValueIn=%.8f nTimeDiff=%d bnCentSecond=%s\n", (double)nValueIn/(double)COIN, nStakeTime, bnCentSecond.ToString());
    }

    CBigNum bnCoinDay = bnCentSecond / 86400;
...
#ifdef ENABLE_WALLET
// LABH: attempt to generate suitable proof-of-stake
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (pfrom->nVersion < MIN_PEER_PROTO_VERSION ||
            (nBestHeight >= HARD_FORK_VERSION_101 && pfrom->nVersion < MIN_PEER_PROTO_VERSION_101))
        {
