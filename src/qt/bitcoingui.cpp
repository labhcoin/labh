...
{
    resize(850+95, 550);
    setWindowTitle(tr("LABH") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
    ...
    centralStackedWidget->addWidget(buyPage);
    ...
    frameBlocksLayout->addSpacerItem(spacer);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a LABH address"));
...
#ifdef Q_OS_MAC
    buyAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_6));
#else
    buyAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_6));
#endif
    tabGroup->addAction(buyAction);
    ...
    connect(buyAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(buyAction, SIGNAL(triggered()), this, SLOT(gotoBuyPage()));

    quitAction = new QAction(tr("E&xit"), this);
    quitAction->setToolTip(tr("Quit application"));
    quitAction->setShortcut(QKeySequence(Qt::CTRL + Qt::Key_Q));
    quitAction->setMenuRole(QAction::QuitRole);
    aboutAction = new QAction(tr("&About LABH"), this);
    aboutAction->setToolTip(tr("Show information about LABH"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for LABH"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("LABH client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("LABH client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to LABH network", "", count));
}
...
void BitcoinGUI::askFee(CAmount nFeeRequired, bool *payFee)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;
...
void BitcoinGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
    ...
    CAmount amount = ttm->index(start, TransactionTableModel::Amount, parent)
...
void BitcoinGUI::gotoBuyPage()
{
    buyAction->setChecked(true);
    centralStackedWidget->setCurrentWidget(buyPage);

    exportAction->setEnabled(false);
    disconnect(exportAction, SIGNAL(triggered()), 0, 0);

    // TODO: check order status, if pending for deposit,
    // reload status otherwise reload package.
    buyPage->show();
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid LABH address or malformed URI parameters."));
    }
...
void BitcoinGUI::updateStakingIcon()
{
    ...
    {
        CAmount nWeight = this->nWeight;
        CAmount nNetworkWeight = GetPoSKernelPS();
