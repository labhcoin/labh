...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        // Min Fee
        CAmount nMinFee = GetMinFee(txDummy, 1, GMF_SEND, nBytes);
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(PAIRTYPE(QString, vector<COutput>) coins, mapCoins)
    {
        ...
        int nChildren = 0;
        int nInputSum = 0;
        CAmount nDisplayWeight = 0;
        CAmount nTxWeightSum = 0;
        CAmount nPotentialStakeSum = 0;
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            // amount
            CAmount nBlockSize = out.tx->vout[out.i].nValue / COIN;
            ...
            // potential stake
            double nPotentialStake = (double)nBlockSize * (GetCoinYearReward(nBestHeight+1)/CENT/100) * ((double)nAge / (86400) / 365);
        ...
        if (treeMode)
        {
            ...
            // Tree mode weight
            itemWalletAddress->setText(COLUMN_WEIGHT, strPad(QString::number(nTxWeightSum), 8, " "));
        }
    }
