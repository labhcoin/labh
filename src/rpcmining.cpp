...
Value getstakesubsidy(const Array& params, bool fHelp)
{
    ...
    return GetProofOfStakeReward(pindexBest, nCoinAge, 0);
}
...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    CAmount nMinWeight, nMaxWeight, nWeight = 0;
    if (pwalletMain)
    ...
    obj.push_back(Pair("blockvalue",       (CAmount)GetProofOfWorkReward(0)));
    obj.push_back(Pair("netmhashps",       GetPoWMHashPS()));
    obj.push_back(Pair("netstakeweight",   GetPoSKernelPS()));
    obj.push_back(Pair("errors",           GetWarnings("statusbar")));
    obj.push_back(Pair("pooledtx",         (uint64_t)mempool.size()));

    weight.push_back(Pair("minimum",       nMinWeight));
    weight.push_back(Pair("maximum",       nMaxWeight));
    weight.push_back(Pair("combined",      nWeight));
    obj.push_back(Pair("stakeweight",      weight));

    obj.push_back(Pair("stakeinterest",    (CAmount)GetCoinYearReward(nBestHeight+1)));
...
Value getstakinginfo(const Array& params, bool fHelp)
{
    ...
    CAmount nMinWeight = 0, nMaxWeight = 0, nWeight = 0;
    if (pwalletMain)
        pwalletMain->GetStakeWeight(nMinWeight, nMaxWeight, nWeight);
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "LABH is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "LABH is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "LABH is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "LABH is downloading blocks...");
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH (CTransaction& tx, pblock->vtx)
    {
        ...
        {
            entry.push_back(Pair("fee", (CAmount)(tx.GetValueIn(mapInputs) - tx.GetValueOut())));
    ...
    result.push_back(Pair("coinbasevalue", pblock->vtx[0].vout[0].nValue));
