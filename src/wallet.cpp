...
void CWalletTx::GetAmounts(list<pair<CTxDestination, CAmount> >& listReceived,
                           list<pair<CTxDestination, CAmount> >& listSent, CAmount& nFee, string& strSentAccount) const
{
    nFee = 0;
    listReceived.clear();
    listSent.clear();
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
                if (!(pcoin->IsSpent(i)) && IsMine(pcoin->vout[i]) &&
                    ...
                    (!IlIII(nBestHeight+1) || (IlIII(nBestHeight+1) && pcoin->lIIII(nBestHeight+1)))) {
                        LogPrint("stake", "%s(): added coin=%s", __func__, pcoin->ToString());
                        vCoins.push_back(COutput(pcoin, i, nDepth));
                    }
        }
    }
...
bool CWallet::SelectCoinsMinConf(CAmount nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet) const
{
    setCoinsRet.clear();
    nValueRet = 0;
...
bool CWallet::SelectCoins(CAmount nTargetValue, unsigned int nSpendTime, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl) const
{
    ...
    boost::function<bool (const CWallet*, CAmount, unsigned int, int, int, std::vector<COutput>, std::set<std::pair<const CWalletTx*,unsigned int> >&, CAmount&)> f = &CWallet::SelectCoinsMinConf;
...
bool CWallet::SelectCoinsForStaking(CAmount nTargetValue, unsigned int nSpendTime, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet) const
{
    vector<COutput> vCoins;
...
bool CWallet::CreateTransaction(const vector<pair<CScript, CAmount> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, int nSplitBlock, const CCoinControl* coinControl)
{
    CAmount nValue = 0;
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                if (!fSplitBlock) {
                    ...
                    BOOST_FOREACH (const PAIRTYPE(CScript, CAmount)& s, vecSend)
                    {
                        for (int nCount = 0; nCount < nSplitBlock; nCount++)
                        {
                            ...
                            {
                                CAmount nReminder = s.second % nSplitBlock;
                ...
                CAmount nMinFee = GetMinFee(wtxNew, 1, GMF_SEND, nBytes);
...
bool CWallet::CreateTransaction(CScript scriptPubKey, CAmount nValue, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRet, const CCoinControl* coinControl)
{
...
bool CWallet::GetStakeWeight(CAmount& nMinWeight, CAmount& nMaxWeight, CAmount& nWeight)
{
    // Choose coins to use
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CAmount nFees, CTransaction& txNew, CKey& key)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        for (unsigned int n=0; n<min(nSearchInterval,(int64_t)nMaxStakeSearchInterval) && !fKernelFound && pindexPrev == pindexBest; n++)
        {
            ...
            if (CheckKernel(pindexPrev, nBits, txNew.nTime - n, prevoutStake, &nBlockTime))
            {
                ...
                CAmount nTotalSize = pcoin.first->vout[pcoin.second].nValue + GetProofOfStakeReward(pIndex0, nCoinAge, nFees);
    ...
    // Calculate coin age reward
    {
        ...
        CAmount nReward = GetProofOfStakeReward(pindexPrev, nCoinAge, nFees);
        if (nReward <= 0)
            return false;
...
bool CWallet::GetStakeWeightFromValue(const int64_t& nTime, const CAmount& nValue, CAmount& nWeight)
{
...
string CWallet::SendMoneyToDestination(const CTxDestination& address, CAmount nValue, CWalletTx& wtxNew, bool fAskFee)
{
    // Check amount
    if (nValue <= 0)
